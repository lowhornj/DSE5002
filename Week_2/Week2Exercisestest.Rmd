...
```{r}
# Your code here
colnames(sales)[colnames(sales) == "?..Row.ID"] <- "Row.ID"
...
x = length(sales$Order.Date) + length(sales$Ship.Date)
avg <- result/x
print(avg)
```
...
sapply(strsplit(Customer.Name, " "), '[',1)
length(grep("Bill", sales$Customer.Name))
```
...
```{r}
# Your code here
barplot(table(sales$Sales == sales$State))
